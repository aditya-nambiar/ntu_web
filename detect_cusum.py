...
def detect_cusum(x,picname,threshold=1, drift=0, ending=False, show=True, ax=None):
    """Cumulative sum algorithm (CUSUM) to detect abrupt changes in data.
    ...
    if show:
        _plot(x,picname, threshold, drift, ending, ax, ta, tai, taf, gp, gn)

    return ta, tai, taf, amp


def _plot(x,picname, threshold, drift, ending, ax, ta, tai, taf, gp, gn):
    ...
    try:
        ...
        ax1.set_ylabel('Average Energy Consumed', fontsize=14)
